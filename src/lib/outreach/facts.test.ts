...
import { COUNTRY_PUBLISH_MIN, DAY_RATE_PUBLISH_MIN } from "../thresholds";
import {
  OUTREACH_LANGUAGES,
  bareCount,
  bestCountry,
...
const CODES: Record<string, string> = { Belgium: "BE", Poland: "PL", Germany: "DE" };

function country(name: string, responses: number, population: Population = "employee"): CountryRow {
  return {
    population,
    code: CODES[name] ?? "ZZ",
    name,
...
/**
 * Day rates are just the contractor population now, not a parallel array. See
 * CLAUDE.md §9: `SiteStats.dayRates` no longer exists.
 */
function rates(name: string, responses: number): CountryRow {
  return country(name, responses, "contractor");
}
...
describe("countryDayRates", () => {
  it("reads a separate population from the salary count, in the same country", () => {
    const s = stats([country("Belgium", 4), rates("Belgium", 7)], 11);
    expect(countryDayRates(s, "BE")).toBe(7);
    expect(countryResponses(s, "BE")).toBe(4);
  });

  it("is zero for a country with no quoted rates", () => {
    expect(countryDayRates(stats([country("Belgium", 4)], 9), "BE")).toBe(0);
    expect(countryDayRates(stats([rates("Poland", 3)], 3), "BE")).toBe(0);
  });

  // The bug this guards: before populations, both counts came from a name
  // match against one flat array. A contractor row and an employee row for the
  // same country now sit side by side, so a lookup that forgets the population
  // returns whichever the aggregation happened to emit first.
  it("never returns a contractor count for a salary token", () => {
    const s = stats([rates("Belgium", 7), country("Belgium", 4)], 11);
    expect(countryResponses(s, "BE")).toBe(4);
  });
});

describe("fillOutreachTokens", () => {
  const live = stats([country("Belgium", 4), country("Poland", 11), rates("Belgium", 7)], 9);
...
describe("bestCountry", () => {
  it("is the country nearest publication, not the newest", () => {
    expect(bestCountry(stats([country("Belgium", 4), country("Poland", 11)]))).toEqual({
      ...
      population: "employee",
      responses: 11,
      needs: COUNTRY_PUBLISH_MIN - 11,
    });
  });

  /**
   * The reason this ranks by share rather than by count. Equal raw numbers are
   * not equal positions once the thresholds differ: 20 day rates are five short
   * of publishing and 20 salaries are forty short. Sorting on the count alone
   * would call these a tie and then break it alphabetically, pointing the next
   * post at the country that is furthest from producing anything.
   */
  it("ranks by share of the threshold that applies, not by raw count", () => {
    const best = bestCountry(stats([country("Belgium", 20), rates("Poland", 20)], 40));
    expect(best).toEqual({
      name: "Poland",
      population: "contractor",
      responses: 20,
      needs: DAY_RATE_PUBLISH_MIN - 20,
    });
  });
